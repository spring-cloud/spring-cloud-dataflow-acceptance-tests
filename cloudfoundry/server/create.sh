...
cat << EOF >> ./scdf-manifest.yml
  ...
  env:
    ...
    SPRING_CLOUD_DEPLOYER_CLOUDFOUNDRY_STREAM_SERVICES: rabbit,cloud-config-server
    SPRING_CLOUD_DEPLOYER_CLOUDFOUNDRY_TASK_SERVICES: mysql,cloud-config-server
    ...
    spring.cloud.dataflow.applicationProperties.stream.TRUST_CERTS: $TRUST_CERTS
    spring.cloud.dataflow.applicationProperties.task.TRUST_CERTS: $TRUST_CERTS
